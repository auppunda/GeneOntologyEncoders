...
os.chdir("/local/auppunda/auppunda")
...
import Elmo.encoder.arg_input as ELMO_arg_input
import Elmo.encoder.data_loader as ELMO_data_loader
import Elmo.encoder.encoder_model as ELMO_encoder_model
...
if args.tree:
  ...
  LabelSamples = ELMO_data_loader.convert_label_desc_to_features(LabelSamples, MAX_SEQ_LEN_LABEL_DEF, tokenizer, all_name_array=label_in_ontology, tokenize_style='bert')
  ...
  LabelSamples = ELMO_data_loader.convert_label_desc_to_features(LabelSamples, MAX_SEQ_LEN_LABEL_DEF, tokenizer, all_name_array=label_to_test, tokenize_style='bert')


GO_loader_for_BERT, GO_name_for_BERT = ELMO_data_loader.label_loader_for_write (LabelSamples,args.batch_size_bert,fp16=False) ## if we fix BERT encoder, then we don't have to worry about batch size, should be able to handle 32 or even 64
...
cosine_loss = ELMO_encoder_model.cosine_distance_loss(1024, args.def_emb_dim, args)
metric_pass_to_joint_model = {'entailment':None, 'cosine':cosine_loss}

#* **** add yes/no classifier to BERT ****
## init joint model
GOEncoder = ELMO_encoder_model.encoder_model (args, metric_pass_to_joint_model[args.metric_option] , elmo, go_dic, **other_params )

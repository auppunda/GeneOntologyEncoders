...
if args.tree:
  ...
  LabelSamples = GCN_data_loader.convert_labels_to_features(LabelSamples, MAX_SEQ_LEN_LABEL_DEF, tokenizer, all_name_array=label_in_ontology, tokenize_style='bert')
  ...
  LabelSamples = GCN_data_loader.convert_labels_to_features(LabelSamples, MAX_SEQ_LEN_LABEL_DEF, tokenizer, all_name_array=label_to_test, tokenize_style='bert')


GO_loader_for_BERT, GO_name_for_BERT = GCN_data_loader.make_label_loader (LabelSamples,args.batch_size_bert,fp16=False) ## if we fix BERT encoder, then we don't have to worry about batch size, should be able to handle 32 or even 64

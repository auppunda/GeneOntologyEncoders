...
pair='YeastPPI3ontology2'
dataDir=$server/$pair'/qnliFormatData17'


for output in cosine.768.reduce300ClsVec ; do # cosine.768.reduce300ClsVec GcnRelu300Cosine cosine.bilstm.300Vec 

  finalDir=$server/$pair'/qnliFormatData17/'$output

  for point in {0..11700..300}
  do 

    echo $point
    scoreFile=$finalDir/'score.'$point'.txt'
    nameOut=$finalDir/'finalscore.'$point'.txt'
    pickleDf=$dataDir/'PPI2testDef.'$point'.txt'
    # pickleDf,scoreFile,nameOut,start
    /local/auppunda/anaconda3/bin/python gather_score.py $pickleDf $scoreFile $nameOut $point
